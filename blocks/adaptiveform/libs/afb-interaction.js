...
export const subscribe = (model, element, additionalHandlers) => {
    ...
    let handlers = {...deafultHandlers(), ...additionalHandlers}
...
export const updateValid = (element, valid, state) => {
    if (element) {
        ...
        if (typeof state.errorMessage !== "string" || state.errorMessage === "") {
            ...
            let errorDiv = getErrorWidget(element)
            if(errorDiv)
                errorDiv.innerHTML = errMessage;
        }
    }
}
...
export const updateErrorMessage = (/** @type {{ querySelector: (arg0: string) => any; }} */ element, /** @type {any} */ errorMessage, /** @type {{ errorMessage: any; }} */ state) => {
    if (element) {
        let errorDiv = getErrorWidget(element)
        if(errorDiv)
            errorDiv.innerHTML = state.errorMessage;
    }
}
...
export const getErrorWidget = (element) => {
    return element.querySelector(`[id$='${Constants.ERROR_MESSAGE}']`)
}
